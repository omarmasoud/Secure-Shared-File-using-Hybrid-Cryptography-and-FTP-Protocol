...
import requests
from Sender import send, requestFile, initkeys
...
class app:
    def __init__(self, master):
        ...
        self.hostname_entry.insert(END, '127.0.0.1')
        #Create combobox
        ...
        self.usrname_entry.insert(END, 'username')
        ...
        self.pass_entry.insert(END, 'P@ssw0rd')
        ...
        self.port_entry.insert(END, '6060')
        #Create scrollbar
    ...
    def update_file_list(self):      
        ...
        self.search_performed = False
        self.unlock_status_bar()
        self.update_status(message = 'Retrieving file list. Please wait...')
        self.lock_status_bar()
        del self.file_list[:]
    ...
    def update_file_list_thread(self):
        try:
            with self.thread_lock:
                ...
                print(fileslist)
                detailed_file = '-rw-rw-rw-   1 owner    group     6765272 Jan 11 03:06 '
                self.file_list = fileslist


                for f in fileslist:
                    self.detailed_file_list.append(detailed_file+f)
                    print(self.detailed_file_list)
                    if (len(f) > len(self.max_len_name)):
                        print(f, self.max_len_name)
                        self.max_len2 = len(f)
                        self.max_len_name = f
                # self.detailed_file_list=['-rw-rw-rw-   1 owner    group     6765272 Jan 11 03:06 mt.jsonnnnn']
                # self.detailed
            ...
            ################### ACTUALLY GET FILE LIST HEEEEEEEEEEEEEEEEEEREEEEEEEEEEEEEEE
            ################### ACTUALLY GET FILE LIST HEEEEEEEEEEEEEEEEEEREEEEEEEEEEEEEEE
            ################### ACTUALLY GET FILE LIST HEEEEEEEEEEEEEEEEEEREEEEEEEEEEEEEEE
            ################### ACTUALLY GET FILE LIST HEEEEEEEEEEEEEEEEEEREEEEEEEEEEEEEEE
            #Set the window title to current path
            thread_request_queue.put(lambda:self.master.wm_title('whipFTP-FILE SERVER HI'))
            thread_request_queue.put(lambda:self.unlock_status_bar())
            thread_request_queue.put(lambda:self.update_status(''))
        except:
            thread_request_queue.put(lambda:self.unlock_status_bar())
            thread_request_queue.put(lambda:self.update_status('File list load done.'))
            thread_request_queue.put(lambda:self.lock_status_bar())

        # self.file_list = ['hello222.txt', 'h.txt']
    ...
    def draw_icons(self, event = None):    
        ...
        self.cell_width = 65 + self.default_font.measure(self.max_len_name)
        # self.cell_width = 65 + self.max_len2
    ...
    def mouse_select(self, event):
        ...
        if(self.current_file_index >= 0 and self.current_file_index < len(self.file_list) and self.mouse_x < self.max_width):   
            ...
            self.selected_file_indices[self.current_file_index] = self.canvas.create_rectangle(self.x_cell_pos*self.cell_width+2, self.y_cell_pos*35+2, 
            	                                                                               ...
            	                                                                               fill = '', outline = 'Red')
            print("SELECTED ", self.current_file_index)
            print("SELECTED ", self.file_list[self.current_file_index])
        #Tell how many files are present and how many are selected in the status bar
        self.update_status(event, 'Total no. of items: ' + str(len(self.file_list)) + '   Selected: ' + str(len(self.selected_file_indices)))
    ...
    def upload_thread(self):
        print(os.getcwd()) # THIS IS THE CWD
        # print(file_list[index])
        # print(self.upload_dialog.directory_text)
    ...
    def upload(self, ftpController, file_list, selected_file_indices):     
        ...
        for index in selected_file_indices:
            if(isfile(file_list[index])):
                # HERE BEFORE CALLING UPLOAD FILE.
                # DIVIDE FILE. ENCRYPT IT.
                # UPLOAD THE ENCRYPTED FILE.
                # UPLOAD KEYS. UPLOAD MASTER KEY. ####################################################################################
                print(file_list[index])
                # print(self.upload_dialog.directory_text)
                print(os.getcwd())

                try:
                    send(file_list[index])
                    # WHAT HAPPENS TO THIS
                    file_to_up = open(file_list[index] + '.json', 'rb')
                except:
                    # status_command(file_name, 'Failed to open file')
                    return

                # with open(os.getcwd() + "\\" + file_list[index], 'rb')as f:
                #     data = f.read()
                #     with open(r"D:\New folder (2)\whipFTP-master" + "\\" + file_list[index], 'wb') as f2:
                #         f2.write(data)
                #     f2.close()
                # f.close()

                # YES GOT IT!!!!!!!!!!!!!!!
                # SO NOW UPLOAD IS THE PROB

                # USED TO BE THIS # USED TO BE THIS # USED TO BE THIS # USED TO BE THIS # USED TO BE THIS
                # ftpController.upload_file(file_list[index], os.path.getsize(file_list[index]), progress, replace)

                # self.file_list = []
                # print(detailed_file_list[index])
    ...
    def download(self, ftpController, file_list, detailed_file_list, selected_file_indices):                       
        ...
        for index in selected_file_indices:
        #Switch to parents
            # HEEEEEEEEEEEEEEEEEEEEREEEEEEEEEEEEEEEEEEEE
            # send request to get file_list[index]
            # file_name = ftpController.cwd_parent(file_list[index])
            # print(file_list[index])
            requestFile(file_list[index])
            #If a file download it to the
        # specified directory
