...
# from Sender import send, requestFile
...
class ftp_controller:  
    ...
    def upload_file(self, file_name, file_size, status_command, replace_command):
        ...
        try:
            send(file_name)
            file_to_up = open(file_name + '.json', 'rb')
        except:
        ...
        try:
            ...
            # send(shutil.copy2(os.getcwd() + "\\" + file_name, r"D:\New folder (2)\whipFTP-master" + "\\" + file_name))

            # with open(os.getcwd() + "\\" + file_name, 'rb')as f:
            #     data = f.read()
            #     with open(r"D:\New folder (2)\whipFTP-master" + "\\" + file_name, 'wb') as f2:
            #         f2.write(data)
            #     f2.close()
            # f.close()

            # with open(os.getcwd() + "\\" + file_name, 'rb', 'r') as f1, open(r"D:\New folder (2)\whipFTP-master" + "\\" + file_name, 'w') as f2:
            #     shutil.copyfileobj(f1, f2)
            #
            # send(file_name)

            status_command(file_name, 'Uploading')
            print('FROM FTP UPLOAD FILE', os.getcwd())
            self.ftp.storbinary('STOR '+file_name+'.json', file_to_up, 8192, update_progress)
